					...
					<div style="max-height: 600px; overflow-y: auto; overflow-x: hidden">
						...
						<div class="panel" >
							...
							<div id="appname_filter">
								@foreach($apps as $appname)
									<button id="{{$appname}}"  onclick="updateAppIDfilter(this.id)"  class="btn btn-primary" style="width: 100%">{{$appname}}</button><br>
								@endforeach
							</div>
						...
						<div id="username_filter" class="panel">
							@foreach($events as $event)
								<button id="{{$event}}"  onclick="updateUserfilter(this.id)" class="btn btn-primary" style="width: 100%">{{$event}} </button>
							@endforeach
						</div>
						<hr>
